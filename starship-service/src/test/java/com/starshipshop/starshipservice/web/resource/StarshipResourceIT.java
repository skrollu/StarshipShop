...
public class StarshipResourceIT {
    ...
    @Test
    void getStarshipByIds_withoutIdsParam_throwsError() throws Exception {
        this.mockMvc.perform(get(BASE_URL + "/in"))
                .andExpect(status().isBadRequest())
        ;
    }

    @Test
    void getStarshipByIds_withoutIds_isOk() throws Exception {
        this.mockMvc.perform(get(BASE_URL + "/in?ids="))
                .andExpect(status().isOk())
        ;
    }

    @Test
    void getStarshipByIds_withIds_givesStarships() throws Exception {
        this.mockMvc.perform(get(BASE_URL + "/in?ids=1,2,3"))
                .andExpect(jsonPath("$._embedded.starships", hasSize(3)))
                .andExpect(jsonPath("$._embedded.starships[0].id", is(1)))
                .andExpect(jsonPath("$._embedded.starships[1].id", is(2)))
                .andExpect(jsonPath("$._embedded.starships[2].id", is(3)))
        ;
    }

    @Test
    void getStarshipByIds_withMultipleTimesTheSameId_givesItOneTime() throws Exception {
        this.mockMvc.perform(get(BASE_URL + "/in?ids=1,1,1,1"))
                .andExpect(jsonPath("$._embedded.starships", hasSize(1)))
                .andExpect(jsonPath("$._embedded.starships[0].id", is(1)))
        ;
    }

    @Test
    void getStarshipByIds_givesSelfLink() throws Exception {
        this.mockMvc.perform(get(BASE_URL + "/in?ids=1"))
                .andExpect(jsonPath("$._links.self.href", endsWith("/in?ids=1")))
        ;
    }
